function vert = posToVert(posn, closed, dir)
...
% vert = posToVert(posn, closed, dir)
%
% INPUTS:
...
%
% dir (optional) {'cw', 'ccw'}: indicates whether the returned vertices
% should be returned in CLOCKWISE ('cw') or COUNTERCLOCKWISE ('ccw')
% orientation. (bbox2Points returns points in a ccw fashion, with no option
% for cw). (Default: 'cw')
%
% OUTPUTS: 
...
elseif lower(dir) == "ccw"
    if xs(1) ~= xs(4)
        xs = xs([1 4 3 2]);
        ys = ys([1 4 3 2]);
    end
else
    warning('posToVert: Ignoring unrecognized DIR instruction.')
end
